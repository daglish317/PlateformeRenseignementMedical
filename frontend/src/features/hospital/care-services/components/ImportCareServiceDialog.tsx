...
import { useImportCareService } from "../hooks/useImportCareService";

interface ImportCareServiceDialogProps {
  structureId: string;
}

export function ImportCareServiceDialog({ structureId }: ImportCareServiceDialogProps) {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const importMutation = useImportCareService(structureId);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Importer des prises en charge</DialogTitle>
          <DialogDescription>
            Fichier Excel (.xlsx) ou CSV avec une colonne <strong>Nom de la prise en charge</strong>.
          </DialogDescription>
        </DialogHeader>
