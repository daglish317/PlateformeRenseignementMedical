...
export function ImportServiceDialog({ structureId }: ImportServiceDialogProps) {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const importMutation = useImportService(structureId);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Fichier Excel (.xlsx) ou CSV avec une colonne <strong>Nom du service</strong>.
          </DialogDescription>
        </DialogHeader>
