...
import { useImportTechnicalPlatform } from "../hooks/useImportTechnicalPlatform";

interface ImportTechnicalPlatformDialogProps {
  structureId: string;
}

export function ImportTechnicalPlatformDialog({ structureId }: ImportTechnicalPlatformDialogProps) {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const importMutation = useImportTechnicalPlatform(structureId);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Importer des plateaux techniques</DialogTitle>
          <DialogDescription>
            Fichier Excel (.xlsx) ou CSV avec une colonne <strong>Nom du plateau technique</strong>.
          </DialogDescription>
        </DialogHeader>
