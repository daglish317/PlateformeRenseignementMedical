...
export default function FavoriteList() {
  ...
  if (!favorites || favorites.length === 0) return <div className="p-8 text-center text-muted-foreground">Aucun favori pour le moment.</div>;
      ...
      {favorites.map((fav) => (
        <article key={fav.id} className="relative overflow-hidden rounded-2xl border bg-card p-4 shadow-sm hover:shadow-md transition-all">
          ...
          <div className="mt-4 flex items-center justify-between border-t pt-4">
            <span className="text-xs text-muted-foreground">Ajouté le {new Date(fav.dateAjout).toLocaleDateString()}</span>
