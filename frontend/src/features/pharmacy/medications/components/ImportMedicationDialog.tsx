...
import { useImportMedication } from "../hooks/useImportMedication";

interface ImportMedicationDialogProps {
  structureId: string;
}

export function ImportMedicationDialog({ structureId }: ImportMedicationDialogProps) {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const importMutation = useImportMedication(structureId);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Fichier Excel (.xlsx) ou CSV avec les colonnes <strong>Nom</strong> (obligatoire) et <strong>Quantité</strong> (optionnelle).
          </DialogDescription>
        </DialogHeader>
