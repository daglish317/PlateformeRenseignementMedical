"""Génération du PDF de la facture d'une vente finalisée (module Facture)."""
...
def generer_facture_pdf(facture):
    """Retourne les octets du PDF de la facture.

    Le document représente fidèlement la facture enregistrée : pharmacie,
    numéro de facture, date, référence de la vente, bénéficiaire, détail des
    produits et total. Le bénéficiaire provient de la facture enregistrée.
    """
    vente = facture.vente
    ...
    infos = Table(
        [
            ...
            ["Bénéficiaire", facture.beneficiaire or "Non renseigné"],
            ["Date", f"{facture.cree_le:%d/%m/%Y}" if facture.cree_le else ""],
        ],
